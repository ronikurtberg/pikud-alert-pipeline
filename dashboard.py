...
def api_filter_options():
    ...
    cities = sorted(name for name in clean_with_shelter if name.count(" ") <= 3 and " - " not in name)
...
@app.route("/api/viz/daily_cities")
def api_viz_daily_cities():
    """City-level alert counts per day — top N cities as stacked bar series."""
    filt, params = build_filter_clause()
    top_n = request.args.get("top", 8, type=int)
    db = get_shared_db()
    if not db:
        return jsonify([])
    # Get top cities by total alerts in the filtered range
    top_cities = [
        r[0]
        for r in db.execute(
            f"""
        SELECT {CITY_DISPLAY} as city_name
        FROM alert_details ad
        JOIN messages m ON ad.msg_id=m.msg_id
        JOIN cities c ON ad.city_id=c.city_id
        WHERE m.message_type='alert' AND m.is_drill=0 {filt}
        GROUP BY {CITY_DISPLAY} ORDER BY COUNT(*) DESC LIMIT ?
    """,
            params + [top_n],
        ).fetchall()
    ]
    ...
    # Get per-day counts for each top city
    placeholders = ",".join("?" * len(top_cities))
    rows = db.execute(
        f"""
        SELECT date(m.datetime_israel) as date,
               {CITY_DISPLAY} as city_name,
               COUNT(*) as cnt
        FROM alert_details ad
        JOIN messages m ON ad.msg_id=m.msg_id
        JOIN cities c ON ad.city_id=c.city_id
        WHERE m.message_type='alert' AND m.is_drill=0
          AND {CITY_DISPLAY} IN ({placeholders}) {filt}
        GROUP BY date, {CITY_DISPLAY}
        ORDER BY date
    """,
        top_cities + params,
    ).fetchall()
    # Build date list and per-city series
    dates = sorted(set(r["date"] for r in rows))
    date_idx = {d: i for i, d in enumerate(dates)}
    series = {}
    for city in top_cities:
        series[city] = [0] * len(dates)
    for r in rows:
        if r["city_name"] in series:
            series[r["city_name"]][date_idx[r["date"]]] = r["cnt"]
    return jsonify({
        "dates": dates,
        "series": [{"city": city, "counts": series[city]} for city in top_cities],
    })
