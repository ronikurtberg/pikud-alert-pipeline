...
CROSS_TABLE_FIELDS = {
    "calculated_dimensions": [
        ...
        {"name": "Shelter_Danger_Level", "context": "Pikud_Alert_Detail",
    ...
    "calculated_measures": [
        ...
        {"name": "Shelter_Danger_Weight", "tableau_formula": 'CASE [Pikud_Alert_Detail].[shelter_time] WHEN "מיידי" THEN 10 WHEN "15 שניות" THEN 8 WHEN "30 שניות" THEN 6 WHEN "45 שניות" THEN 4 WHEN "דקה" THEN 2 WHEN "דקה וחצי" THEN 1 WHEN "3 דקות" THEN 0.5 ELSE 3 END',
         "purpose": "Row-level danger weight by shelter time (10=immediate, 0.5=3min, 3=aircraft default). Auto-SUMs when aggregated.",
         "level": "Row"},
        {"name": "Civilian_Danger_Score", "tableau_formula": 'SUM([Shelter_Danger_Weight])',
         "purpose": "Total danger score — higher means more alerts with less shelter time",
         "depends_on": "Shelter_Danger_Weight"},
        {"name": "Avg_Danger_Per_Alert", "tableau_formula": 'SUM(CASE [Pikud_Alert_Detail].[shelter_time] WHEN "מיידי" THEN 10 WHEN "15 שניות" THEN 8 WHEN "30 שניות" THEN 6 WHEN "45 שניות" THEN 4 WHEN "דקה" THEN 2 WHEN "דקה וחצי" THEN 1 WHEN "3 דקות" THEN 0.5 ELSE 3 END) / COUNT([Pikud_Alert_Detail].[id])',
         "purpose": "Average severity per alert (1-10 scale). Higher = less shelter time on average."},
        {"name": "Immediate_Danger_Rate", "tableau_formula": 'COUNTD(IF [Pikud_Alert_Detail].[shelter_time] = "מיידי" THEN [Pikud_Alert_Detail].[id] END) / COUNT([Pikud_Alert_Detail].[id]) * 100',
         "purpose": "Percentage of alerts with zero shelter time (מיידי)"},
        {"name": "Night_Attack_Rate", "tableau_formula": 'COUNTD(IF DATEPART(\'hour\', [Israel_DateTime]) >= 0 AND DATEPART(\'hour\', [Israel_DateTime]) < 6 THEN [Pikud_Alert_Detail].[id] END) / COUNT([Pikud_Alert_Detail].[id])',
         "purpose": "Fraction of alerts between midnight and 6 AM Israel time",
         "depends_on": "Israel_DateTime"},
        {"name": "Days_With_Sirens", "tableau_formula": 'COUNTD(DATE([Israel_DateTime]))',
         "purpose": "Distinct calendar days with at least one alert",
         "depends_on": "Israel_DateTime"},
    ],
    "semantic_metrics": [
        {"name": "Days_Under_Fire", "label": "Days Under Fire",
         "measure": "Days_With_Sirens", "time_dimension": "Israel_DateTime",
         "time_grains": ["Day", "Week", "Month", "Quarter", "Year"],
         "description": "Total siren activation events. Each distinct message = one event.",
         "sentiment": "up_is_bad"},
        {"name": "Multi_Front_Pressure", "label": "Multi-Front Pressure",
         "measure": "Zones_Affected", "time_dimension": "Israel_DateTime",
         "additional_dimensions": ["Pikud_Zone.zone_name"],
         "time_grains": ["Day", "Week", "Month", "Quarter", "Year"],
         "description": "Defense zones under fire. Israel has 36 total — above 25 = nationwide.",
         "sentiment": "up_is_bad"},
        {"name": "Immediate_Danger_Pct", "label": "Immediate Danger %",
         "measure": "Immediate_Danger_Rate", "time_dimension": "Israel_DateTime",
         ...
         "sentiment": "up_is_bad"},
        {"name": "Night_Terror_Index", "label": "Night Terror Index",
         "measure": "Night_Attack_Rate", "time_dimension": "Israel_DateTime",
         ...
         "sentiment": "up_is_bad"},
        {"name": "Cities_Under_Fire", "label": "Cities Under Fire",
         "measure": "Cities_Affected", "time_dimension": "Israel_DateTime",
         "additional_dimensions": ["City_Display_Name"],
         "time_grains": ["Day", "Week", "Month", "Quarter", "Year"],
         "description": "Distinct cities hearing sirens in period.",
         "sentiment": "up_is_bad"},
        {"name": "Avg_Danger_Level", "label": "Avg Danger Level",
         "measure": "Avg_Danger_Per_Alert", "time_dimension": "Israel_DateTime",
