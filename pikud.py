...
def _apply_city_prefix_inheritance(raw_items: list[str]) -> list[str]:
    ...
    Prefix inheritance only applies to known bare suffix tokens — Hebrew letter
    codes (א, יב, טז…) or known neighborhood/district words (מזרח, מרינה…).
    Real standalone city names (רחובות, רמלה…) always reset the prefix.

    All produced names are dash-normalized so "אשדוד -יב" and "אשדוד - יב"
    become the same canonical form "אשדוד - יב".
    """
    # Known suffix tokens that legitimately inherit a city prefix.
    # Hebrew letter codes (single/double letters) are matched by regex below.
    _KNOWN_SUFFIXES = {
        "מזרח", "מערב", "דרום", "צפון", "מרכז",
        "מרינה", "סיטי", "נמל", "ביצרון", "גת", "חצור",
        "שתולים", "סלעית", "עדנים", "עולש", "עזריאל",
        "פורת", "פרדסיה", "צופים", "צופית", "קלנסווה",
        "הכרמל", "נשר", "יגור", "רמות", "תירוש", "שפיר"
        "עמיקם", "אחיהוד", "ברדלה",
    }
    # Hebrew letter codes: 1-2 char tokens made only of Hebrew letters (aleph-bet)
    _LETTER_CODE_RE = re.compile(r"^[\u05d0-\u05ea]{1,2}$")

    def _is_bare_suffix(token: str) -> bool:
        return token in _KNOWN_SUFFIXES or bool(_LETTER_CODE_RE.match(token))
    ...
    for item in raw_items:
        ...
        elif current_prefix and " " not in cn and _is_bare_suffix(cn):
            # Only inherit prefix for known bare neighborhood/district suffix tokens
            result.append(current_prefix + cn)
        else:
            # Standalone city name or unknown token: reset prefix group
            current_prefix = ""
            result.append(cn)
    return result
...
def build_database(ver_dir: str, db_path: str, version: str) -> bool:
    ...
    # Apply manual canonical overrides for boundary compound zones
    from dashboard_app.city_translations_manual import CANONICAL_OVERRIDES
    override_count = 0
    for compound_name, primary_name in CANONICAL_OVERRIDES.items():
        if compound_name in name_to_id:
            conn.execute(
                "UPDATE cities SET canonical_name=? WHERE city_id=?",
                (primary_name, name_to_id[compound_name]),
            )
            override_count += 1
    if override_count:
        conn.commit()
        print(f"  Canonical overrides: {override_count} boundary zones mapped to primary cities")
