...
class TestFilterOptions:
    def test_returns_all_types(self, client):
        ...
        assert len(d["cities"]) > 1400  # ~1,500 clean cities after parser bug fix
        assert len(d["zones"]) > 30
        assert "rockets" in d["alert_types"]
        assert "aircraft" in d["alert_types"]

    def test_no_compound_cities_in_filter_options(self, client):
        """Compound boundary names like 'ראשון לציון - רחובות' must be excluded from city autocomplete."""
        r = client.get("/api/filter_options")
        d = json.loads(r.data)
        for city in d["cities"]:
            assert " - " not in city, f"Compound city leaked into filter options: {city}"
...
class TestVisualizationEndpoints:
    ...
    def test_viz_with_hour_filter(self, client):
        """Hour filter should return fewer alerts than unfiltered."""
        r_all = client.get("/api/filtered_counts")
        r_hour = client.get("/api/filtered_counts?hour_from=12&hour_to=14")
        assert r_hour.status_code == 200
        d_all = json.loads(r_all.data)
        d_hour = json.loads(r_hour.data)
        assert d_hour["city_alerts"] > 0
        assert d_hour["city_alerts"] < d_all["city_alerts"]

    def test_hour_filter_on_daily(self, client):
        """Daily endpoint should respect hour_from/hour_to."""
        r = client.get("/api/viz/daily?hour_from=8&hour_to=10")
        assert r.status_code == 200
        d = json.loads(r.data)
        assert isinstance(d, list)
        assert len(d) > 0
...
class TestDailyCities:
    def test_returns_expected_structure(self, client):
        r = client.get("/api/viz/daily_cities")
        assert r.status_code == 200
        d = json.loads(r.data)
        assert "dates" in d
        assert "series" in d
    ...
    def test_series_has_city_and_counts(self, client):
        r = client.get("/api/viz/daily_cities")
        d = json.loads(r.data)
        for s in d["series"]:
            assert "city" in s
            assert "counts" in s
            assert len(s["counts"]) == len(d["dates"])

    def test_top_n_parameter(self, client):
        r6 = client.get("/api/viz/daily_cities?top=6")
        r12 = client.get("/api/viz/daily_cities?top=12")
        d6 = json.loads(r6.data)
        d12 = json.loads(r12.data)
        assert len(d6["series"]) <= 6
        assert len(d12["series"]) <= 12
        assert len(d12["series"]) >= len(d6["series"])

    def test_with_date_filter(self, client):
        r = client.get("/api/viz/daily_cities?date_from=2024-01-01&date_to=2024-12-31")
        assert r.status_code == 200
        d = json.loads(r.data)
        assert "dates" in d
        for date in d["dates"]:
    ...
    def test_with_hour_filter(self, client):
        r = client.get("/api/viz/daily_cities?hour_from=6&hour_to=18")
        assert r.status_code == 200
        d = json.loads(r.data)
        assert "dates" in d
        assert "series" in d
    ...
    def test_counts_align_with_dates(self, client):
        r = client.get("/api/viz/daily_cities?top=4")
        d = json.loads(r.data)
        n = len(d["dates"])
        for s in d["series"]:
            assert len(s["counts"]) == n, f"City {s['city']} counts length mismatch"
...
class TestSummary:
    ...
    def test_summary_api_unfiltered(self, client):
        ...
        d = json.loads(r.data)
        assert d["attack_events"] > 0
        assert d["total_cities"] > 1800  # ~1,853 after parser bug fix eliminated spurious compound names
